...
## 5.3
- **Spiel-Icons: jetzt wirklich repariert.** 5.2 hatte die Ursache verfehlt. Gemessen wurden drei echte Fehler: (1) Die in 5.2 eingebaute Wiederholung konnte nie anlaufen – `Start-PlaceIconLoad` wurde ausschließlich beim *Erstellen* einer Zeile gerufen, der Fehlerzähler `PlaceIconFails` wurde also gefüllt, aber nie wieder gelesen. Ein einziger Fehlversuch ließ den Rahmen damit für immer leer, und weil das Ersatzbild erst ab dem 3. Versuch gezeichnet wurde, kam auch das nie. Neu: `Update-PlaceIconRetries` läuft bei jedem Oberflächen-Takt. (2) Ein nicht veröffentlichtes Place hat bei Roblox gar kein Bild; dafür wurde ein Studio-Logo aus dem Internet geladen – ohne Netz/hinter Firewall oder Proxy blieb der Rahmen leer. Jetzt wird das Ersatzbild **lokal gezeichnet**, ganz ohne Netz, und es erscheint schon beim **ersten** Fehlschlag. (3) Der Ladevorgang lief in einem frisch erzeugten Runspace und benutzte dort `Invoke-RestMethod`/`Test-Path`/`Move-Item`; lädt so ein Runspace die Zusatzmodule nicht nach, kommt nie ein Ergebnis zurück. Der Ladevorgang benutzt jetzt nur noch .NET und kennt drei Wege zum Bild (gameId → Thumbnails-API, placeId → Universe-Id → Thumbnails-API, placeId → Place-Gameicons-API).
- **Icons zusätzlich:** Bilder werden über einen Speicherstrom geladen (WPF hielt die Datei sonst offen und lieferte beim gleichen Pfad hartnäckig das alte, gecachte Bild), und das Mosaik von „Alle Places“ wird nach dem Neubau der Zeile wieder gezeichnet – vorher blockierte die Signatur der alten Zeile den Neuaufbau und der Rahmen blieb leer.
- **Arena-Verlauf: jetzt wirklich benutzbar.** Ebenfalls drei Fehler: (1) Der Verlauf hing an der *Sitzungs-Id*; jedes Plugin-Neuladen und jeder Studio-Neustart erzeugt eine neue Sitzung – das Fenster war sofort wieder leer, und das Aufräumen toter Sitzungen (120 s) löschte den Verlauf zusätzlich mit. Er hängt jetzt an einem stabilen **Place-Schlüssel** (placeId, sonst Place-Name) und wird beim Aufräumen nicht mehr angefasst. (2) Der Verlauf lebte nur im Arbeitsspeicher – er wird jetzt je Place nach `%LOCALAPPDATA%\ArenaRobloxBridge\history` gesichert und beim nächsten Start wieder geladen (400 Aktionen je Place, älter als 30 Tage wird verworfen). (3) Die Texte zeigten weiter Platzhalter statt echter Werte, weil die Hilfsfunktionen einen Parameter namens `$args` hatten – das ist in PowerShell eine **automatische Variable**, die den übergebenen Wert wortlos mit einer leeren Liste überschreibt. Der Parameter heißt jetzt `$ToolArgs`, damit stehen Objektnamen, Skriptnamen, Zeilen- und Stückzahlen wirklich im Verlauf.
- **Verlauf zusätzlich:** Der Fensteraufbau liegt in einem try/catch (ein Fehler hat vorher den Zeitgeber getötet und den Verlauf eingefroren – ohne eine Spur im Log), die Änderungs-Signatur erkennt auch Aktualisierungen innerhalb derselben Sekunde, der Löschen-Knopf entfernt auch die gespeicherte Datei, und die „noch nichts passiert“-Karte verschwindet, sobald es echte Einträge gibt.
- Nach dem Update Roblox Studio einmal neu starten, damit das Plugin **5.3** geladen wird.
