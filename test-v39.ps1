# ============================================================================
# Arena Roblox Bridge 3.9.8 - Logik-Tests (Mock, ohne Studio / ohne UI)
# ----------------------------------------------------------------------------
# Ausfuehren auf dem PC (Windows PowerShell 5.1):
#   powershell.exe -NoProfile -ExecutionPolicy Bypass -File test-v39.ps1
#
# Die Tests pruefen die aktuelle 3.9.8-Logik OHNE Roblox Studio:
...
#   8. Versions-Konsistenz (3.9.8 ueberall)
#   9. GET-API, Kopier-Bestaetigung und Autostart-Selbst-Update (3.9.8)
# ============================================================================
...
Write-Host '=== Arena Bridge 3.9.8 Logik-Tests ===' -ForegroundColor Cyan
...
# ----------------------------------------------------------------------------
Write-Host '`n8) Version 3.9.8 ueberall' -ForegroundColor Yellow
Assert ($text.Contains("# Arena Roblox Bridge  -  Version 3.9.8")) 'Changelog-Kopf'
Assert ($text.Contains("DocsVersion     = '3.9.8'")) 'DocsVersion'
Assert ($text.Contains('local ARENA_VERSION  = "3.9.8"')) 'ARENA_VERSION (Plugin)'
Assert ($text.Contains('Arena Studio Bridge - Studio Plugin  (Version 3.9.8)')) 'Plugin-Kommentar'
Assert ($text.Contains("version = '3.9.8'")) 'Manifest-Version'
Assert ($text.Contains("serverVersion = '3.9.8'")) 'serverVersion'
Assert ($text.Contains('$versionText = ' + "'3.9.8'")) 'Show-UpdateNotice-Fallback'
Assert ($text.Contains('Text="Arena Roblox Bridge - Version 3.9.8"')) 'Einstellungs-Fusszeile'
$bridgeVersions = [regex]::Matches($text, "bridgeVersion = '(\d+\.\d+\.\d+)'")
Assert ($bridgeVersions.Count -eq 3 -and @($bridgeVersions | Where-Object { $_.Groups[1].Value -ne '3.9.8' }).Count -eq 0) 'bridgeVersion (3x)'
$versionJson = Get-Content (Join-Path $root 'version.json') -Raw -Encoding UTF8 | ConvertFrom-Json
Assert ($versionJson.version -eq '3.9.8') 'version.json'
Assert ($versionJson.notes.Length -gt 50) 'version.json hat Neuigkeiten-Text'

# ----------------------------------------------------------------------------
# 9) Neue 3.9.8-Features
# ----------------------------------------------------------------------------
Write-Host '`n9) GET-API, Kopier-Bestaetigung, Autostart-Update' -ForegroundColor Yellow

# --- GET-API --------------------------------------------------------------
Assert ($text.Contains('GET-Vollsteuerung (Version 3.9.8)')) 'GET-Koerperaufbau im Router vorhanden'
$getBlockStart = $text.IndexOf('GET-Vollsteuerung (Version 3.9.8)')
