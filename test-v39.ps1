# ============================================================================
# Arena Roblox Bridge 5.0.2 - Logik-Tests (Mock, ohne Studio / ohne UI)
# ----------------------------------------------------------------------------
# Ausfuehren auf dem PC (Windows PowerShell 5.1):
#   powershell.exe -NoProfile -ExecutionPolicy Bypass -File test-v39.ps1
#
# Die Tests pruefen die aktuelle 5.0.2-Logik OHNE Roblox Studio:
...
#   8. Versions-Konsistenz (5.0.2 ueberall)
...
Write-Host '=== Arena Bridge 5.0.2 Logik-Tests ===' -ForegroundColor Cyan
...
# ----------------------------------------------------------------------------
Write-Host '`n8) Version 5.0.2 ueberall' -ForegroundColor Yellow
Assert ($text.Contains("# Arena Roblox Bridge  -  Version 5.0.2")) 'Changelog-Kopf'
Assert ($text.Contains("DocsVersion     = '5.0.2'")) 'DocsVersion'
Assert ($text.Contains('local ARENA_VERSION  = "5.0.2"')) 'ARENA_VERSION (Plugin)'
Assert ($text.Contains('Arena Studio Bridge - Studio Plugin  (Version 5.0.2)')) 'Plugin-Kommentar'
Assert ($text.Contains("version = '5.0.2'")) 'Manifest-Version'
Assert ($text.Contains("serverVersion = '5.0.2'")) 'serverVersion'
Assert ($text.Contains('$versionText = ' + "'5.0.2'")) 'Show-UpdateNotice-Fallback'
Assert ($text.Contains('Text="Arena Roblox Bridge - Version 5.0.2"')) 'Einstellungs-Fusszeile'
$bridgeVersions = [regex]::Matches($text, "bridgeVersion = '(\d+\.\d+\.\d+)'")
Assert ($bridgeVersions.Count -eq 3 -and @($bridgeVersions | Where-Object { $_.Groups[1].Value -ne '5.0.2' }).Count -eq 0) 'bridgeVersion (3x)'
$versionJson = Get-Content (Join-Path $root 'version.json') -Raw -Encoding UTF8 | ConvertFrom-Json
Assert ($versionJson.version -eq '5.0.2') 'version.json'
